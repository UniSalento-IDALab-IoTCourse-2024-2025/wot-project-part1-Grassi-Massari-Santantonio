...
const MenuDragoneRosso = ({ navigation }: any) => {
  const [cart, setCart] = useState<MenuItem[]>([]);
  const [menuItems, setMenuItems] = useState<MenuItem[]>([]);
  const [vendorId, setVendorId] = useState<string | null>(null);
  const [cartCount, setCartCount] = useState(0);
  const restaurantId = "Dragone Rosso";
  ...
  useEffect(() => {
    const fetchVendorId = async () => {
      try {
        ...
        const selected = vendors.find((v: Vendor) => v.name === "Dragone Rosso");
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Menù Dragone Rosso</Text>
      <FlatList
        data={menuItems}
...
export default MenuDragoneRosso;