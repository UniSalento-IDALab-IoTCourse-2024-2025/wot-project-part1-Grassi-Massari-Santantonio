...
const MenuLaPerlaDorata = ({ navigation }: any) => {
  const [cart, setCart] = useState<MenuItem[]>([]);
  const [menuItems, setMenuItems] = useState<MenuItem[]>([]);
  const [vendorId, setVendorId] = useState<string | null>(null);
  const [cartCount, setCartCount] = useState(0);
  const restaurantId = "La Perla Dorata";
  ...
  useEffect(() => {
    const fetchVendorId = async () => {
      try {
        ...
        const selected = vendors.find((v: Vendor) => v.name === "La Perla Dorata");
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Menù La Perla Dorata</Text>
      <FlatList
        data={menuItems}
...
export default MenuLaPerlaDorata;